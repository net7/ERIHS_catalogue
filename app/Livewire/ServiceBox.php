...
class ServiceBox extends Component implements HasActions, HasForms
{
    ...
    public $showViewDetailsButton = true;
    ...
    public function addToCart(): void
    {
        ...
        if ($this->service) {
            ERIHSCartService::addItem($this->service->id);

            $this->dispatch('refreshCart');

            Notification::make()
                ->title('Service "' . $this->service->title . '" added to proposal')
                ->success()
                ->send();
        }
    }
    ...
    {
        if ($this->service) {
            ERIHSCartService::removeItem($this->service->id);

            $this->dispatch('refreshCart');

            Notification::make()
                ->title('Service "' . $this->service->title . '" removed from proposal')
                ->success()
                ->send();
        }
    }
    ...
    public function removeFromCartAction(): Action
    {
        ...
        return Action::make('removeFromCart')
            ...
            ->modalHeading('Remove "' . $this->service->title . '" from proposal')
    ...
    public function addToFavourites()
    {
        if ($this->service) {
            ...
            ERIHSFavouriteService::addItem($this->service->id);

            $this->dispatch('refreshFavourites');

            Notification::make()
                ->title('Service "' . $this->service->title . ' " added to favourites')
                ->success()
                ->send();
        }
    }
    ...
    {
        if ($this->service) {
            ERIHSFavouriteService::removeItem($this->service->id);

            $this->dispatch('refreshFavourites');

            Notification::make()
                ->title('Service "' . $this->service->title . '" removed from favourites')
                ->success()
                ->send();
        }
    }
    ...
    public function viewDetailsAction(): Action
    {
        ...
        return Action::make('viewDetails')
            ->url(fn (): string => route('service', ['id' => $this->service->id]))
            // ->openUrlInNewTab()
            ->link()
            ->extraAttributes([
                'class' => 'py-0',
    ...
    public function render()
    {
        ...
        return view('livewire.service-box');
    }
}
